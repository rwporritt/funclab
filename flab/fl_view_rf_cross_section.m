function fl_view_rf_cross_section(ProjectDirectory,ProjectFile,FuncLabPreferences)
...
uicontrol('Style','Text','Parent',tmpfig,'Units','Normalized','Position',[0.025 0.6 0.3 0.2],'String','Box width (degrees):','FontSize',12);
uicontrol('Style','Edit','Parent',tmpfig,'Units','Normalized','Position',[0.34 0.6 0.2 0.2],'String','1.0','FontSize',12,'Tag','width_control_edit_box','Callback',@setprofilewidth);
%Components = {'Radial RF', 'Transverse RF', 'Vertical','Radial','Transverse'};
Components = {'Radial RF', 'Transverse RF'};
uicontrol('Style','Popupmenu','Parent',tmpfig,'Units','Normalized','Position',[0.6 0.6 0.35 0.2],'String',Components,'Value',1,'FontSize',12,'Tag','component_popupmenu','Callback',@setprofiletype);

uicontrol('Style','Text','Parent',tmpfig,'Units','Normalized','Position',[0.025 0.2 0.3 0.2],'String','Gain:','FontSize',12);
uicontrol('Style','Edit','Parent',tmpfig,'Units','Normalized','Position',[0.34 0.2 0.2 0.2],'String','1.0','FontSize',12,'Tag','gain_control_edit_box','Callback',@setprofilegain);

waitbutton = uicontrol('Style','Pushbutton','Parent',tmpfig,'Units','Normalized','Position',[0.6 0.2 0.35 0.2],'String','Confirm','FontSize',12,'Callback','gcbf.UserData=1; close');
waitfor(waitbutton,'Userdata');
...
drawnow
OutputFigureDir = [ProjectDirectory filesep 'FIGURES' filesep 'RF_Cross_Section_Time']
...
DataMatrix = zeros(length(TimeArray),nStations);
...
for istation = 1:nStations
    ...
    CorrectedSeismogramAmps = mapPsSeis2time_1d( RFTimes, RFAmps, rayparams, dz, oneDVel(:,1), oneDVel(:,2), oneDVel(:,3), TimeArray );
    stackArray = zeros(1,length(TimeArray));
...
ylim(ax,[StartTimeArray EndTimeArray])
set(ax,'YDir','Reverse')
grid(ax,'on')
grid(ax,'minor')
box(ax,'on')
...
for istation=1:nStations
   ...
   vp = [PFilledTrace(:,istation)'+n; TimeAxis'];
   vn = [NFilledTrace(:,istation)'+n; TimeAxis'];
   idx = length(vp);
   vvp = vp;
   vvn = vn;
   vvp(1,idx+1) = n;
   ...
   line(ax,(Amplitudes(:,istation) + n),TimeArray,'Color',[0 0 0],'LineWidth',1)
end
