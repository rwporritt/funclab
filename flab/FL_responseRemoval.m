%% function [DeconnedTrace, ierr] = FL_responseRemoval( inputTrace, lowFreqFloor, responseLevel )
...
if length(DeconnedTrace) ~= 3
    fprintf('FL_responseRemoval error. Need 3 component data!\n');
    ierr=1;
    return
end
...
for itrace=1:3
    ...
    DeconnedTrace{itrace}.data = FL_applyHannTaper(DeconnedTrace{itrace}.data,5);
    ...
    responseSpectrum = FL_generate_response(DeconnedTrace{itrace}.sacpz, freqArray, lowFreqFloor);
    ...
    if strcmp(responseLevel,'Pole-Zero to Velocity')
    %   % one differentiation
        DeconnnedTrace = FL_timeDifferentiate( DeconnedTrace );
    elseif strcmp(responseLevel, 'Pole-Zero to Acceleration')
    %   % two differentations
        DeconnedTrace = FL_timeDifferentiate( DeconnedTrace );
        DeconnedTrace = FL_timeDifferentiate( DeconnedTrace );
    end
end
