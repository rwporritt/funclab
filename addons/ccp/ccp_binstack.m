function ccp_binstack(cbo,eventdata,handles)
%CCP_BINSTACK Plots the CCP stack
%   CCP_BINSTACK is a callback function that plots the CCP stack for the
...
if isempty(strmatch(TableType,'CCP Tables'))
    disp(['Not the right table type: ' TableType])
    return
end

Record = get(h.MainGui.records_ls,'Value');
CurrentRecords = evalin('base','CurrentRecords');   % 06/12/2011: REPLACED CALLING "ENSEMBLE" WITH CALLING THE CURRENTRECORDS
...
if length(ValueIndices) ~= length(RRFBootMean)
    ...
    for j = 1:length(ValueIndices)
        ...
        elseif j == length(ValueIndices)
            if exist('K')
                YAxis = BootAxis(K); % 06/12/2011: REPLACED RRFBOOTAXIS WITH BOOTAXIS
    ...
    YAxis = BootAxis;    % 06/12/2011: REPLACED RRFBOOTAXIS WITH BOOTAXIS
...
set(ax,'TickDir','out','XLim',[-RFAmpCut RFAmpCut],'YLim',[CurrentRecords{Record,7} CurrentRecords{Record,8}],'Xcolor',[.1 .1 .1],...
    'Position',[.2 .05 .65 .85],'XAxisLocation','top','Ycolor',[.1 .1 .1],'YDir','reverse','FontUnits','normalized','FontSize',0.021);  % 06/12/2011: REPLACED Ensemble.BeginDepth WITH CurrentRecords{Record,7} AND Ensemble.EndDepth WITH CurrentRecords{Record,8}
yl = ylabel(ax,'Depth (km)','Units','normalized','FontUnits','normalized','FontSize',0.021);
TitleText = ['CCP Bin Centered at ' num2str(CurrentRecords{Record,14},'%02.2f') ' Lat., ' num2str(CurrentRecords{Record,15},'%03.2f') ' Lon.'];  % 06/12/2011: REPLACED Ensemble.BinLatitude WITH CurrentRecords{Record,14} AND Ensemble.BinLongitude WITH CurrentRecords{Record,15}
...
axis ([-RFAmpCut RFAmpCut CurrentRecords{Record,7} CurrentRecords{Record,8}]);  % 06/12/2011: REPLACED Ensemble.BeginDepth WITH CurrentRecords{Record,7} AND Ensemble.EndDepth WITH CurrentRecords{Record,8}
