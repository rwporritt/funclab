function ccp_1D_raytracing(cbo,eventdata,handles)
%CCP_1D_RAYTRACING 1D ray tracing
%   CCP_1D_RAYTRACING is a callback function that computes the 1D ray
...
% guidata(WaitGui,H);
WaitGui = waitbar(0,'Please wait while performing 1D ray tracing for ACTIVE records in the dataset');
...
if CurrentSubsetIndex == 1
    ...
    CCPDataDir = fullfile(ProjectDirectory, SetName, 'CCPData');
...
% OLD
% MagicNumber = 15000;    % 05/07/2011 CHANGED MAX NUMBER OF ARRAY ELEMENTS FROM 25,000 T0 15,000 TO AVOID OUT OF MEMORY ERRORS
% Lets blow this up:
%MagicNumber = 2000000;
...
Record = 0;
SwitchCount = 0;
NActive = size(ActiveStrings,1);
NUpdate = floor(NActive / 100);
for n = 1:size(ActiveStrings,1)
    ...
    % This might not be working correction (RWP 05.18.2017)
    if Record >= MagicNumber
        ...
        if ~exist(CCPMatFile,'file')
            save(CCPMatFile,NewName1,NewName2);
        else
            save([CCPMatFile,NewName1,NewName2,'-append']);
        end
        clear MidPoints RayMatrix RayTracingMatrix RayRecords
        clear(NewName1)
        clear(NewName2) % 05/08/2011 DELETED EXTRA CLEAR COMMAND FROM LINE 197
    end
    if mod(n, NUpdate) == 0
        waitbar(n/NActive,WaitGui);
    end
end
...
if SwitchCount == 0
    ...
    save(CCPMatFile,'RayMatrix_01','MidPoints_01');
    SwitchCount = 1;
else    % 05/22/2011    ADDED ELSE STATEMENT TO SAVE LAST MATRIX
...
CTime = clock;
NewLog{1} = sprintf('%02.0f/%02.0f/%04.0f %02.0f:%02.0f:%02.0f - CCP Stacking: 1D Ray Tracing set %s',CTime(2),CTime(3),CTime(1),CTime(4),CTime(5),CTime(6), SetName);
...
delete(WaitGui)
set(h.MainGui.message_t,'String',['Message: CCP Stacking: 1D ray tracing took ' num2str(t_temp) ' seconds (or ' num2str((t_temp)/60) ' minutes).'])
