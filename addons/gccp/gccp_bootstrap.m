function [BootMean,BootSigma,Peaks410,Peaks660,Amps410,Amps660] = gccp_bootstrap(Matrix,Weights,YAxis,BootstrapNumber,TraceNumber)
%GCCP_BOOTSTRAP Bootstrap resampling and stacking
%   GCCP_BOOTSTRAP(Matrix,BootstrapNumber,TraceNumber) stacks the
...
if TotalTraces == 1
    ...
    for n = 1:BootstrapNumber
        ...
        %----------------------------------------------------------------------
        %rand('state',sum(100*clock)) % out of date syntax
        rng('shuffle','twister')
...
%             BootMatrix(:,n) = ignoreNaN(BootWeights,@mean,2)/sum(Weights(~isnan(Weights(BootstrapIndices))));
            ...
            BootMatrix(:,n) = ignoreNaN(BootWeights,@sum,2)./ignoreNaN(Weights(:,BootstrapIndices),@sum,2);
        end
    end
