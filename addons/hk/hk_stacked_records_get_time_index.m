function [TimeMinOut, TimeMaxOut] = hk_stacked_records_get_time_index(index)
%HK_STACKED_RECORDS_GET_TIME_INDEX Plot station records stacked
...
%   Asks the user for a min and max Ps conversion time for the Moho via
%   ginput
...
Station = RecordMetadataStrings{index,2};
...
uicontrol('Parent',fig,'Style','text','Units','normalized','String','Please click min/max time for Ps conversion',...
    'Position',[.1 .85 .8 .05],'BackgroundColor',[1 1 1],'FontWeight','Bold','FontSize',14);
...
if TimeInput(1) < TimeInput(2)
    TimeMinOut = TimeInput(1);
    TimeMaxOut = TimeInput(2);
else
    TimeMinOut = TimeInput(2);
    TimeMaxOut = TimeInput(1);
end
