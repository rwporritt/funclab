function ccpv2_do_ray_tracing_parallel_StoP(params)
%% CCPV2_DO_RAY_TRACING_PARALLEL_STOP
%    Does the ray tracing for the ccpv2
%    This version is designed for parallel computing of S to P converted
%    waves
%    
%    Input Parameters:
...
% Prototype - parallel computing not working yet
...
% Setup the input cell arrays for mapSpSeis2depth_1d
...
jray = 0;
for iray = 1:nrecords
    % Prepare call to mapSpSeis2depth_1d from processRFmatlab
...
[AllDepths, AllEpos, AllNpos, AllSeisOutR] = mapSpSeis2depth_1d(AllTime, AllSeisR, AllP, AllBackaz, ...
    params.DepthIncrement, oneDVel(:,1), oneDVel(:,2), oneDVel(:,3));

[~, ~, ~, AllSeisOutT] = mapSpSeis2depth_1d(AllTime, AllSeisT, AllP, AllBackaz, ...
    params.DepthIncrement, oneDVel(:,1), oneDVel(:,2), oneDVel(:,3));
...
for iter = 1:params.NIterations
    ...
    for iray=1:nActiveRecords
        ...
        [deps, epos, npos, seisout] = mapSpSeis2depth_1d(TmpTime, TmpSeis, rayparam, backaz, ...
