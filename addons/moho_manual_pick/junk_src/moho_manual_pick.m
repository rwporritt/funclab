...
%MOHO_MANUAL_PICK Start the Moho_auto_pick GUI
%   MOHO_MANUAL_PICK initiates the manual Moho picking GUI.  
...
%
%   Based on the auto pick method, but pops up graphics to allow the user
%   to click the moho. Again, we can either use the ray info or the ccp. If
%   we use the ray info, we need a fast option that just loops through all
%   the stations one after the other and another option where the user uses
%   the station table or a pop up menu to select one station at a time. For
%   the ccp, we need to step through a series of cross sections and click
%   along the cross sections. The pick points can then be gathered up into
%   a scattered interpolant.
%   Also, use the RF cross section methods to allow picking. For these
%   though, we have to pin the picks to the nearest station location.
%   Basically, any method we have of viewing RFs in depth, should have an
%   associated picking method
%
%   Addon on variables:
%     MohoManualPickStrings - {'RayInfo Moho Mat File', 'CCPV2 Moho Mat File'}
%     MohoManualPickRIDoubles - (RIlongitude RIlatitude RIelevation RIthickness
%     RIMohoAmplitude)
%     MohoManualPickCCPDoubles - (CCPlongitude CCPlatitude CCPelevation CCPthicknessRT
...
%--------------------------------------------------------------------------
MohoManualPickGui = dialog('Name','Moho Manual Picking','Resize','off','NumberTitle','off','Tag','dialogbox','Units','characters','WindowStyle','normal',...
...
% method into another.
setappdata(MohoManualPickGui,'MinThickness',10.0);
setappdata(MohoManualPickGui,'MaxThickness',60.0);
setappdata(MohoManualPickGui,'DeltaThickness',1.0);
setappdata(MohoManualPickGui,'MinAmplitude',0.5);
setappdata(MohoManualPickGui,'RayInfoFile','None');
setappdata(MohoManualPickGui,'CCPFile','None');
...
DefaultMinThickness_s = sprintf('%1.1f',getappdata(MohoManualPickGui,'MinThickness'));
DefaultMaxThickness_s = sprintf('%1.1f',getappdata(MohoManualPickGui,'MaxThickness'));
DefaultDeltaThickness_s = sprintf('%1.1f',getappdata(MohoManualPickGui,'DeltaThickness'));
DefaultMinAmplitude_s = sprintf('%1.1f',getappdata(MohoManualPickGui,'MinAmplitude'));
...
if exist('MohoManualPickCell','var')
    if size(MohoManualPickCell,1) >= CurrentSubsetIndex
        MohoManualPickStrings = MohoManualPickCell{CurrentSubsetIndex};
        if ~isempty(MohoManualPickStrings{1})
            if exist(MohoManualPickStrings{1},'file')
                PlotRIMohoEnable = 'on';
            end
            if length(MohoManualPickStrings) >= 2
                if exist(MohoManualPickStrings{2},'file')
                    PlotCCPMohoEnable = 'on';
                end
            else
                MohoManualPickStrings{2} = '';
            end
        end
    end
end
...
%--------------------------------------------------------------------------
MPlotting = uimenu(MohoManualPickGui,'Label','Plotting');
uimenu(MPlotting,'Label','Plot Ray Info Moho','Tag','moho_manual_pick_ri_plot_m','Enable',PlotRIMohoEnable,'Callback',{@plot_ri_manual_moho_callback});
uimenu(MPlotting,'Label','Plot CCP Moho','Tag','moho_manual_pick_ccp_plot_m','Enable',PlotCCPMohoEnable,'Callback',{@plot_ccp_manual_moho_callback});
MExport = uimenu(MohoManualPickGui,'Label','Export');
uimenu(MExport,'Label','Export Ray Info Moho','Tag','moho_manual_pick_ri_export_m','Enable',PlotRIMohoEnable,'Callback',{@export_ri_manual_moho_callback});
uimenu(MExport,'Label','Export CCP Moho','Tag','moho_manual_pick_ccp_export_m','Enable',PlotCCPMohoEnable,'Callback',{@export_ccp_manual_moho_callback});
MHelp = uimenu(MohoManualPickGui,'Label','Help');
uimenu(MHelp,'Label','Help','Tag','moho_manual_pick_help_m','Enable','on','Callback',{@moho_manual_pick_help_callback});
...
% Title
uicontrol ('Parent',MohoManualPickGui,'Style','text','Units','characters','Tag','title_t','String','Moho manual picking',...
...
uicontrol('Parent',MohoManualPickGui,'Style','Pushbutton','Units','Characters','Tag','moho_manual_pick_close_pb','String','Cancel',...
    'Position',CloseButtonPosition,'FontUnits','normalized','FontSize',0.4,'ForegroundColor',AccentColor,'FontWeight','bold',...
    'Callback',{@close_moho_manual_pick_Callback});

CloseButtonPosition = [21 2 18 2];
do_ray_info_button = uicontrol('Parent',MohoManualPickGui,'Style','Pushbutton','Units','Characters','Tag','run_ray_info_pb','String','Pick Ray Info',...
    'Position',CloseButtonPosition,'FontUnits','normalized','FontSize',0.4,'ForegroundColor',AccentColor,'FontWeight','bold','Enable','off',...
    'Callback',{@do_ray_info_moho_manual_pick_Callback});

CloseButtonPosition = [40 2 18 2];
do_ccp_button = uicontrol('Parent',MohoManualPickGui,'Style','Pushbutton','Units','Characters','Tag','run_ccp_pb','String','Pick CCP',...
    'Position',CloseButtonPosition,'FontUnits','normalized','FontSize',0.4,'ForegroundColor',AccentColor,'FontWeight','bold','Enable','off',...
    'Callback',{@do_ccp_moho_manual_pick_Callback});
...
% button for go ccp
uicontrol('Parent',MohoManualPickGui,'Style','Text','Units','Characters','Tag','min_thickness_t','String','Min Thickness:',...
    'Position',ParamTextPos1,'FontUnits','normalized','FontSize',TextFontSize,'HorizontalAlignment','left','BackgroundColor','white');
uicontrol('Parent',MohoManualPickGui,'Style','Edit','Units','Characters','Tag','min_thickness_e','String',DefaultMinThickness_s,...
...
uicontrol('Parent',MohoManualPickGui,'Style','Text','Units','Characters','Tag','max_thickness_t','String','Max Thickness:',...
    'Position',ParamTextPos1,'FontUnits','normalized','FontSize',TextFontSize,'HorizontalAlignment','left','BackgroundColor','white');
uicontrol('Parent',MohoManualPickGui,'Style','Edit','Units','Characters','Tag','max_thickness_e','String',DefaultMaxThickness_s,...
    'Position',ParamOptionPos1,'FontUnits','normalized','FontSize',EditFontSize,'HorizontalAlignment','left','BackgroundColor','white',...
    'Callback','MaxThick = str2num(get(gcbo,''String''));setappdata(gcbf,''MaxThickness'',MaxThick);');

ParamTextPos1(2) = 16;
ParamOptionPos1(2) = 16;
uicontrol('Parent',MohoManualPickGui,'Style','Text','Units','Characters','Tag','delta_thickness_t','String','Delta Thickness:',...
    'Position',ParamTextPos1,'FontUnits','normalized','FontSize',TextFontSize,'HorizontalAlignment','left','BackgroundColor','white');
uicontrol('Parent',MohoManualPickGui,'Style','Edit','Units','Characters','Tag','delta_thickness_e','String',DefaultDeltaThickness_s,...
    'Position',ParamOptionPos1,'FontUnits','normalized','FontSize',EditFontSize,'HorizontalAlignment','left','BackgroundColor','white',...
    'Callback','DeltaThick = str2num(get(gcbo,''String''));setappdata(gcbf,''DeltaThickness'',DeltaThick);');

ParamTextPos1(2) = 14;
ParamOptionPos1(2) = 14;
uicontrol('Parent',MohoManualPickGui,'Style','Text','Units','Characters','Tag','min_amplitude_t','String','Min amplitude:',...
    'Position',ParamTextPos1,'FontUnits','normalized','FontSize',TextFontSize,'HorizontalAlignment','left','BackgroundColor','white');
uicontrol('Parent',MohoManualPickGui,'Style','Edit','Units','Characters','Tag','min_amplitude_e','String',DefaultMinAmplitude_s,...
    'Position',ParamOptionPos1,'FontUnits','normalized','FontSize',EditFontSize,'HorizontalAlignment','left','BackgroundColor','white',...
    'Callback','MinAmp = str2num(get(gcbo,''String''));setappdata(gcbf,''MinAmplitude'',MinAmp);');

ParamTextPos1(2) = 12;
ParamOptionPos1(2) = 12;
uicontrol('Parent',MohoManualPickGui,'Style','Text','Units','Characters','Tag','ray_info_file_t','String','Ray Info File:',...
    'Position',ParamTextPos1,'FontUnits','normalized','FontSize',TextFontSize,'HorizontalAlignment','left','BackgroundColor','white');
longposition = ParamOptionPos1;
longposition(2) = 10;
longposition(1) = ParamTextPos1(1);
longposition(3) = ParamOptionPos1(1) + ParamOptionPos1(3);
rayinfo_edit = uicontrol('Parent',MohoManualPickGui,'Style','Edit','Units','Characters','Tag','ray_info_file_e','String','',...
...
uicontrol('Parent',MohoManualPickGui,'Style','Pushbutton','Units','Characters','Tag','rayinfo_file_pb','String','Choose File',...
...
longposition(1) = ParamTextPos1(1);
longposition(3) = ParamOptionPos1(1) + ParamOptionPos1(3);
ccp_edit = uicontrol('Parent',MohoManualPickGui,'Style','Edit','Units','Characters','Tag','ccp_file_e','String','',...
    'Position',longposition,'FontUnits','normalized','FontSize',EditFontSize,'HorizontalAlignment','left','BackgroundColor','white',...
    'Callback',{@set_ccp_file_from_edit,do_ccp_button});
uicontrol('Parent',MohoManualPickGui,'Style','Text','Units','Characters','Tag','ccp_file_t','String','CCPV2 File:',...
    'Position',ParamTextPos1,'FontUnits','normalized','FontSize',TextFontSize,'HorizontalAlignment','left','BackgroundColor','white');
uicontrol('Parent',MohoManualPickGui,'Style','Pushbutton','Units','Characters','Tag','ccp_file_pb','String','Choose File',...
    'Position',ParamOptionPos1,'FontUnits','normalized','FontSize',0.4,'ForegroundColor',AccentColor,'FontWeight','bold',...
