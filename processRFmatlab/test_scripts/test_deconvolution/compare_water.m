...
%--------------------
% Optimized water level using preceeding noise
fprintf('\nUsing leading noise for water level\n')
% Find the arrival time
arrival = hdrZ.times.a - hdrZ.times.b;
[rf4,rms] = makeRFwaterOptimized( rseis, zseis, tdel, dt, nt, arrival, wlevel*100, f0, true);

t=cputime;
for i=1:1e2,
  makeRFwaterOptimized( rseis, zseis, tdel, dt, nt, arrival, wlevel*100, f0, false); 
end
timeTakenFor100 = cputime-t
...
h4 = plot(time,rf4,'--r'); hold on;
h5 = plot(time,zeros(1,length(rf4)),'k--'); hold on;
...
disp('The optimized version uses the peak of a smoothed pre-arrival window power spectra to determine the water level.')
